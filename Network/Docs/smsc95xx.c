...
static void smsc95xx_set_multicast(struct net_device *netdev)
{
	...
	unsigned long flags;
	printk(KERN_INFO "SMSC: Multicast called\n");
	...
	if (dev->net->flags & IFF_PROMISC) {
		...
		printk(KERN_INFO "SMSC: Set multicast, else if\n");
		...
		printk(KERN_INFO "SMSC: Set multicast, else called\n");
...
{
	u32 intdata;
	printk(KERN_INFO "SMSC: smsc status called\n");
	...
	if (intdata & INT_ENP_PHY_INT_) {
		printk(KERN_INFO "SMSC: smsc status calling kevent\n");
...
static int smsc95xx_reset(struct usbnet *dev)
{
	...
	int ret = 0, timeout;
	printk(KERN_INFO "SMSC Reset Called\n");
	...
	printk(KERN_INFO "SMSC Reset: Set mac address calling\n");
	...
	printk(KERN_INFO "SMSC Reset: Invoking Multicast\n");
...
{
	printk(KERN_INFO "RX fixup called\n");
	while (skb->len > 0) {
		...
		if (unlikely(header & RX_STS_ES_)) {
			...
			printk(KERN_INFO "Else part of RX fixup\n");
			/* ETH_FRAME_LEN + 4(CRC) + 2(COE) + 4(Vlan) */
			if (unlikely(size > (ETH_FRAME_LEN + 12))) {
				netif_dbg(dev, rx_err, dev->net,
					  "size err header=0x%08x\n", header);
			printk(KERN_INFO "RX fixup : 1 - Returning 0\n");
				return 0;
			}

			/* last frame in this batch */
			if (skb->len == size) {
				printk(KERN_INFO "RX fixup : last part\n");
				...
				printk(KERN_INFO "RX fixup : ax_skb\n");
			ax_skb = skb_clone(skb, GFP_ATOMIC);
			if (unlikely(!ax_skb)) {
				netdev_warn(dev->net, "Error allocating skb\n");
			printk(KERN_INFO "RX fixup : 2 - Returning 0\n");
				return 0;
			}
			...
			printk(KERN_INFO "RX fixup : calling usbnet_skb_return\n");

...
void serial_omap_start_tx(struct uart_port *port, char *ch)
{
	struct uart_omap_port *up = to_uart_omap_port(port);
	serial_omap_enable_ier_thri(up);
	t = ch;
	wait_event_interruptible(wq_tx, flag_tx == 1);
	flag_tx = 0;
	//transmit_chars(up, 0);
}
#if 1
...
void serial_read(struct uart_omap_port *up, unsigned char *t)
{
	unsigned char ch = 0;

	up->ier |= (UART_IER_RDI | UART_IER_RLSI);
	serial_out(up, UART_IER, up->ier);
	wait_event_interruptible(wq_rx, flag == 1);
	up->ier &= ~UART_IER_RLSI;
	serial_out(up, UART_IER, up->ier);
	flag = 0;
...
#endif
static void
serial_set_params(struct uart_port *port)
{
	struct uart_omap_port *up = to_uart_omap_port(port);
	...
	unsigned int c_cflag = 0xcbd;
	unsigned int c_iflag = 0x500;
	//up->ier = 0;
#if 1
	...
	baud = 9600; //uart_get_baud_rate(port, termios, old, 0, port->uartclk/13);
	//quot = serial_omap_get_divisor(port, baud);
	quot = 384;

...
static void serial_read(struct uart_omap_port *up, unsigned char *t)
{
	unsigned char ch = 0;
...
serial_set_params(struct uart_port *port)
{
	...
	baud = 115200; //uart_get_baud_rate(port, termios, old, 0, port->uartclk/13);
...
static int serial_open(struct uart_port *port)
{
	struct uart_omap_port *up = to_uart_omap_port(port);
	unsigned long flags = 0;
	int retval;
